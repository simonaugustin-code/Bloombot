...
const FAQ: { q: string; a: string; keys: string[] }[] = [
  ...
  { q: "Aké spôsoby platby ponúkate?", a: "Platba kartou vopred alebo dobierka.", keys: ["platba", "spôsoby", "dobierka", "karta"] },
  {
  ...
  },
  {
    ...
    a: "Potvrdenú objednávku nemožno upravovať. Ak ešte nebola odoslaná, vieme ju stornovať a môžete vytvoriť novú.",
  ...
  { q: "Posielate balíky opakovane?", a: "Preposielanie neprevzatých balíkov neposkytujeme. Vytvorte novú objednávku.", keys: ["preposielanie", "neprevzaté"] },
  { q: "Kedy bude akcia?", a: "Sledujte naše sociálne siete; ak niečo beží, uvidíte to na webe/IG.", keys: ["akcia", "zľava", "sale"] },
];
...
function findFaqAnswer(question: string) {
  ...
  if (best.s >= 2) return FAQ[best.i]!.a;
  return null;
}
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!openaiRes.ok || !openaiRes.body) throw new Error(`OpenAI HTTP ${openaiRes.status}`);
